...
Сущность контекста Catalog. На Tier B — **не DDD-агрегат** (нет aggregate root, инварианты проверяются в handler'е, не в модели); плоская модель. Контекст-секции (язык, роли, события-контракт, интеграции) — в [корне `catalog-spec.md`](../catalog-spec.md).

## 1. Доменная модель

Product — карточка товара продавца. Плоская модель (Tier B): отдельных сущностей и Value Object'ов нет, состояние — атрибуты карточки. Хранение (схема, типы) — [корень → Техническая реализация](../catalog-spec.md#11-техническая-реализация).
...
Product не публикует доменных событий (Tier B; см. [корень → Доменные события](../catalog-spec.md#5-доменные-события)).
